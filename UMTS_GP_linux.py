...
import GPy
import random
import get_loc_linux  # Need to change to _linux
...
print len(dbdata2),' Length of data set'
...
a = np.array(mXa)
b = np.array(mXb)
# Normlizing the loation data from the selected cells.
if True:
...
print end - start, " seconds to constrain all parameters to be positive"

# Add statement to plot inline
# The magic is not needed for the pure python version
#get_ipython().magic(u'pylab --no-import-all inline')
# optimize and plot
start = time.time()
m.optimize('tnc', max_f_eval = 50)
print end - start, " seconds to optimize with 50 function evals"
m.plot()
